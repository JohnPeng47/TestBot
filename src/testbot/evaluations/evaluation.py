...
@click.pass_context
@with_db_session
def build_commits(ctx: click.Context, repo_path: str, session: Session = None):
...
@click.option("--num-files", type=int, help="Filter by number of files")
@click.option("--num-test-files", type=int, help="Filter by number of test files")
@click.option("--sha", help="Filter by commit SHA")
@click.option("--diff-bytes", type=int, help="Filter by diff size in bytes")
@with_db_session
def list_commits(repo, num_files, num_test_files, sha, diff_bytes, session: Session = None):
